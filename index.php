...
array_walk($dictionary, function (&$value) {
    $value = trim($value);
});

if(isset($_GET['new_word'])){
    $_SESSION['baseWord'] = '';
    header("Location:" . $url=strtok($_SERVER["REQUEST_URI"],'?'));
    exit();
}


if(empty($_SESSION['baseWord'])) {
    $_SESSION['baseWord'] = $dictionary[array_rand($dictionary)];
}
        ...
        var createWordContainer = function(){
            ...
            for(var i in word){
                ...
                letter.onclick   = function(){
                    ...
                    this.setAttribute("isactive", Math.pow(isActive, isActive) - isActive);
